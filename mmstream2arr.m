function xy=mmstream2arr(x,y,u,v,x0,y0,mark,step)
...
xy=zeros(1,2); % create cells for output
...
for k=1:N0 % find streamlines given [x0, y0] pairs
    ...
    switch mark
        case 's'
            xy=local_getstream(x,y,u,v,ilim,jlim,x0(k),y0(k),step);
        case 'e'
            xy=local_getstream(x,y,-u,-v,ilim,jlim,x0(k),y0(k),step);
            xy=xy(end:-1:1,:); % make stream flow downhillm
        case 'o'
            [xy,ic]=local_getstream(x,y,u,v,ilim,jlim,x0(k),y0(k),step);
            if ~ic % go other direction only if streamline is not closed
                xye=local_getstream(x,y,-u,-v,ilim,jlim,x0(k),y0(k),step);
                xy=[xye(end:-1:2,:);xy]; % make stream flows downhill
            end
    end
xy(isnan(xy(:,1)),:)=[];
end
